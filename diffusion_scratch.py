...
# =========================================================
epochs = 50
num_timesteps = 1000  # FIX 1: Reduced from 1500 → standard DDPM value (Ho et al. 2020)
learning_rate = 2e-4  # FIX 2: Reduced from 1e-3 → primary cause of divergence (loss exploded to 19825 at epoch 20)
base_channel = 64
batch_size = 128
...
def visualize_forward_diffusion():
    timesteps_to_show = [0, 250, 500, 750, 999]  # FIX 3: Updated to match num_timesteps=1000 (was [0,375,750,1125,1499])
...
if __name__ == "__main__":
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(  # FIX 4: LR scheduler added — smoothly decays LR to stabilise training
    ...
    for epoch in range(epochs):
        ...
        for xo, _ in dataloader:
            ...
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=0.5)  # FIX 5: Tightened from 1.0 → 0.5 for extra stability
            optimizer.step()

            running_loss += loss.item()

        avg_loss = running_loss / len(dataloader)
        scheduler.step()  # FIX 6: Step the cosine LR scheduler once per epoch
